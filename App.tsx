...
import { 
  ...
  getSpidaCommDrop, 
  _to_feet_ts, 
  normalizeScid as commonNormalizeScid, 
...
const App: React.FC = () => {
  ...
  const [spidaAliasTable, setSpidaAliasTable] = useState<Record<string, string>>({});
  ...
  const [rawKatapultJsonFull, setRawKatapultJsonFull] = useState<KatapultJsonFormat | null>(null);
  const [katapultBirthmarks, setKatapultBirthmarks] = useState<Record<string, KatapultBirthmarkData>>({});
  ...
  const [isStatusSectionVisible, setIsStatusSectionVisible] = useState(false);
  ...
  const toggleStatusSection = useCallback(() => {
    setIsStatusSectionVisible(prev => !prev);
  }, []);
  ...
  const handleExportAIReport = useCallback(async (reportType: 'executive' | 'technical' | 'field') => {
    if (processedPoles.length === 0) {
      setStatusMessage('No data available for AI report generation.');
      return;
    }

    setStatusMessage(`Generating AI-powered ${reportType} report...`);
    
    try {
      const geminiService = (await import('./services/geminiService')).GeminiService;
      ...
      const projectData = {
        ...
        poleData: processedPoles.slice(0, 100),
        hasComparisonRun: true,
        projectInfo: {
          spidaFileName,
          katapultFileName,
          totalProcessedPoles: processedPoles.length,
          generatedAt: new Date().toISOString()
        }
      };

      const aiReport = await service.generateDetailedReport(projectData, reportType);
      
      // Create downloadable report
      const reportContent = `
QuiC AI-Generated ${reportType.charAt(0).toUpperCase() + reportType.slice(1)} Report
Generated: ${new Date().toLocaleString()}
Project: ${spidaFileName || 'Unknown'} vs ${katapultFileName || 'Unknown'}

${aiReport}

---
Generated by QuiC AI Analytics powered by Google Gemini
      `;

      const blob = new Blob([reportContent], { type: 'text/plain' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `QuiC_AI_${reportType}_Report_${new Date().getTime()}.txt`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(link.href);
      
      setStatusMessage(`AI-powered ${reportType} report generated and downloaded.`);
    } catch (error) {
      console.error('Error generating AI report:', error);
      setStatusMessage(`Error generating AI report: ${(error as Error).message}`);
    }
  }, [processedPoles, comparisonStats, spidaFileName, katapultFileName]);
  ...
  const exportSection = (
     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3">
      ...
      {/* AI-Powered Export Section */}
      {processedPoles.length > 0 && (
        <>
          <div className="sm:col-span-2 lg:col-span-1 xl:col-span-2 border-t border-slate-600 pt-3 mt-2">
            <div className="flex items-center space-x-2 mb-3">
              <svg className="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
              <span className="text-emerald-400 text-sm font-semibold">AI-Powered Reports</span>
            </div>
          </div>
          
          <IconButton 
            onClick={() => handleExportAIReport('executive')} 
            disabled={processedPoles.length === 0 || isComparing}
            className="w-full justify-center text-sm bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-blue-500/30 hover:border-blue-400/50"
            icon={
          ...
          <IconButton 
            onClick={() => handleExportAIReport('technical')} 
            disabled={processedPoles.length === 0 || isComparing}
            className="w-full justify-center text-sm bg-gradient-to-r from-emerald-600/20 to-cyan-600/20 border-emerald-500/30 hover:border-emerald-400/50"
            icon={
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <IconButton 
            onClick={() => handleExportAIReport('field')} 
            disabled={processedPoles.length === 0 || isComparing}
            className="w-full justify-center text-sm sm:col-span-2 lg:col-span-1 xl:col-span-2 bg-gradient-to-r from-orange-600/20 to-red-600/20 border-orange-500/30 hover:border-orange-400/50"
            icon={
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
